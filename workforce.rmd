...
event_counts <- event_counts %>%
  mutate(GroupedDescription = case_when(
    grepl("^ALACAD", `Event Clearance Description`) ~ "ALACAD",
...
ggplot(event_counts_summary, aes(x = GroupedDescription, y = TotalCount, group = 1)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8)) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2))

# Create the bar plot
event_counts <- data_frame(event_counts)
ggplot(aes(event_counts$`Event Clearance Description`)) +
   geom_line()

ggplot(event_counts, aes(x = `Event Clearance Description`)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
# Optionally, abbreviate the descriptions
event_counts$ShortDescription <- abbreviate(event_counts$`Event Clearance Description`, minlength = 10)

# Plotting
ggplot(event_counts, aes(x = ShortDescription, y = count, group = 1)) +
  geom_line() +
