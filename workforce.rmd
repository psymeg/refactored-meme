...
#library(wrapr) #orderv dataframes 
#library(forcats) #order graphs ## who needs graphs
...
#Police 911 Data
dl <- "Seattle_Police_Department_911_Incident_Response.csv"
if(file.exists(dl)){
  ...
  #this wont work as it looks like kaggle now require an auth token
  #if you do download you will need to unzip the archive file produced
...
``` {r 911dataSummary, echo=FALSE, results="hide", message=FALSE, warning=FALSE}
...
#Split Incident Location into Latitude_911 and Longitude_911
#Seattle_911 <- Seattle_911 %>%
#  mutate(Incident.Location = gsub("[()]", "", Incident.Location)) %>%
#  separate(Incident.Location, into = c("Latitude_911", "Longitude_911"), sep = ", ") 

#Convert Latitude_911/Longitude_911 entries to numeric
Seattle_911$Longitude_911 <- as.numeric(Seattle_911$Longitude_911)
Seattle_911$Latitude_911 <- as.numeric(Seattle_911$Latitude_911)

Seattle_Weather <- Seattle_Weather %>%
  mutate(DATE = as.Date(DATE, format = "%Y-%m-%d"))
min_date <- min(Seattle_Weather$DATE, na.rm = TRUE)
max_date <- max(Seattle_Weather$DATE, na.rm = TRUE)
#number of events between min(Seattle_Weather$Date) and max(Seattle_Weather$Date)
#min_date <- min(as.POSIXct(Seattle_Weather$DATE, format = "%m/%d/%Y %I:%M:%S %p", tz = "America/Los_Angeles"), na.rm = TRUE)
#max_date <- max(as.POSIXct(Seattle_Weather$DATE, format = "%m/%d/%Y %I:%M:%S %p", tz = "America/Los_Angeles"), na.rm = TRUE)
weather_event_count <- Seattle_911 %>%
  filter(Event.Clearance.Date >= min_date & Event.Clearance.Date <= max_date) %>%
  count()
```

### Seattle 911 Dataset Investigation
...
The number of events that fall within the timeframe of the Weather data available are: `r (weather_event_count)`.


``` {r graphEventClearaceDescription, echo=FALSE, results="hide", message=FALSE, warning=TRUE}
...
group_event_counts <- event_counts %>%
...
event_counts_30 <- group_event_counts[1:30,] %>%
  arrange(GroupedDescription)
# View the result
print(event_counts)
...
#group_event_counts %>%
 ## group_by(GroupedDescription) %>%
  #reframe(quant90  = quantile(total_count), probs = .9)
#event_count_graph <- ggplot(event_counts,aes(GroupedDescription)) +
 # geom_bar(stat = identity, fill = "orange") +
#  coord_flip() 
#Sevent_count_graph  

#event_count_graph <- ggplot(group_event_counts, aes(x = GroupedDescription, y = fct_infreq(total_count))) +
...
``` {r SplitStations, echo=FALSE, results="hide", message=FALSE, warning=TRUE}
